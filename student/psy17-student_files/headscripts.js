...
/* Functions for the navigation tab hover stuff */
var TabHolderNavDivs_OnMouseOver = function (divId) {

	if (document.getElementById("AdministrationLinks") && divId != "AdministrationLinks") {
		fade("AdministrationLinks", { duration: 0 });
	};
	if (document.getElementById("CoursesLinks") && divId != "CoursesLinks") {
		fade("CoursesLinks", { duration: 0 });
	};
	if (document.getElementById("PortfoliosLinks") && divId != "PortfoliosLinks") {
		fade("PortfoliosLinks", { duration: 0 });
	};
	if (document.getElementById("ProjectsLinks") && divId != "ProjectsLinks") {
		fade("ProjectsLinks", { duration: 0 });
	};

	appear(divId, { duration: 0 });
};
var TabHolderNavDivs_OnMouseOut = function (divId) {
	fade(divId, { delay: 0.5, duration: 0 });
};
var TabHolderNavDivs_OnMouseOverDiv = function (divId) {
	appear(divId, { queue: 'end', duration: 0 });
};
var TabHolderNavDivs_OnMouseOutDiv = function (divId) {
	fade(divId, { delay: 0.5, duration: 0 });
};

/* Check/uncheck all toggle */
function doSelectAll(formName, field) {
  var i;
  if (! eval(document.forms[formName].checkAll.checked)) {
    for (i=0; i<field.length; i++) {
      if (! field[i].disabled) { field[i].checked = false; }
    }
  } else {
    for (i=0; i<field.length; i++) {
